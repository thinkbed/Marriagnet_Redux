...
export class MarriagnetServerPHP extends Backend {
   ...
   */
   initialize (token) {
     ...
     this.BASE_URL = CONFIG.backend.myserverLocal
           ? CONFIG.MYSERVER.local.url
           : CONFIG.MYSERVER.remote.url
   }
    ...
    return await this._fetch({
      ...
      .then((res) => {
        ...
        (res.code === 209)) {
          ...
          throw new Error({code: 404, error: 'unknown error from Marriagnet.com'})
        }
   ...
   */
   async _fetch (opts) {
    ...
    return await fetch(this.BASE_URL + opts.url, reqOpts)
  }
}

export let myserver = new MarriagnetServerPHP()